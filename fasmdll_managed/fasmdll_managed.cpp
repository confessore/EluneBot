...
namespace Fasm
{
	...
	array<Byte>^ ManagedFasm::Assemble(ValueType^ origin)
	{
		if (origin != IntPtr::Zero)
		{
			...
			String^ format = gcnew String("org {0}");
			m_AssemblyString->Insert(0, String::Format(format, args) + Environment::NewLine);
		}
		return ManagedFasm::Assemble(m_AssemblyString->ToString(), m_MemorySize, m_PassLimit);
	}
